...
山区洪涝灾害易造成道路和通信设施同时中断，有限的无人机与电池还须承担多服务区、多时限的物资配送。如何在复杂地形中兼顾安全载荷、及时交付、连续通信与分区独立执行，是应急调度面临的核心问题。本文从地形约束下的飞行能量计算和异构车辆路径优化入手，结合题给30\,m数字高程模型、15个服务区、80个不可拆货箱及设备参数，依次建立单点往返组批、多机多架次运输、运输与中继联合调度、固定任务分区配置四层模型，并以逐箱记录、资源时间轴和通信证书核验结果。

针对问题一，首先按地形最高点确定巡航高度，建立载荷相关的往返时间与能耗模型；其次由安全能量约束求出三种机型对各服务区的最大载荷；最后枚举可行货箱组合，以“架次数、能耗、累计作业时间”为字典序目标，用状态压缩动态规划求解，并以整数规划交叉验证。在20\%返航安全余量下，80箱组成18个直接往返架次，总能耗59.1313\,kWh，累计作业时间9.104\,h；15个服务区的最少架次数和固定架次数下的最小能耗均通过交叉核验。

针对问题二，首先允许单架次访问多个服务区，生成满足质量、体积和返航电量限制的候选路线；其次建立含交付时限、实体运输机和共享电池占用的整数排程模型；最后对所得路线优化任务时序，并以大邻域重建检验改进空间。所得方案以24个运输架次完成80箱交付，其中2架次跨区，全部期望时刻及硬截止时刻均满足；运输能耗69.5805\,kWh，最后返航时刻为6997.523\,s。已选24条路线的整数秒排程具有最优性证书，路线选择不作全局最优性声称。

针对问题三，首先沿问题二的运输轨迹进行视距遮挡判定和双向链路预算，识别直连失效区段；其次构造候选中继位置及两跳时空覆盖关系，建立运输架次、中继服务窗与能源组件的联合调度模型；最后采用束搜索、时间窗口细化和连续区间证书验证。正式方案包含24个运输架次和5个中继架次，运输与中继总能耗73.8354\,kWh，联合完工时间8772.339\,s；80箱均唯一交付，硬截止违约及资源冲突为零。1317个证明区间覆盖全部通信需求时段，未出现未决区间或通信中断。

针对问题四，首先由问题三的共同运输架次和实际中继保障关系建立任务依赖图，将15个服务区归并为3个不可拆单元；其次在保持原任务及其时刻不变的前提下穷举两组和三组划分；最后按资源占用区间计算各组独立执行的并发需求。两组推荐将S001单列，需补充1架C型运输机和1块C型电池；三组唯一方案还须将S006单列，需各补充2件。两类分组均不能仅凭现有库存按原时刻表独立执行。

模型在问题一10\%--20\%返航余量范围内保持18架次组批，表现出一定稳定性；安全余量继续提高时出现离散跃迁。通信附加损耗分析显示原证书对链路裕量变化较敏感，实际部署须重新校核。模型的物理计算、分层求解与逐级验收流程可随地形、需求及设备参数更新后用于同类救援调度，但问题二、三的可行方案不等同于全局最优解。

\keywords{山区洪涝灾害\quad 无人机应急运输\quad 共享电池调度\quad 通信中继\quad 任务分区}
\end{abstract}
