...
\end{equation}
交付偏移$\delta_{pb}$按同一时间线累计到箱$b$完成交接为止。候选只有在起飞质量、体积、逐段航迹和$E_p\le(1-\rho_{g_p})E_{g_p}^{\rm use}$均成立时才保留；即使立即开始准备也无法满足硬时限的候选同样剔除。这样做比对整条路线只使用一个平均载荷更准确，特别是跨区架次在首站卸货后的后续航段。
